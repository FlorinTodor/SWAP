...
function build_image(){
  case $1 in
    ...
    traefik_balanceador)
      ...
      docker build --no-cache -t flotodor-traefik_balanceador-image:p3 -f ./P3-flotodor-traefik/DockerfileTraefik_balanceador .
      ;;
    envoy_balanceador)
      docker rmi flotodor-envoy_balanceador-image:p3 -f
      docker build --no-cache -t flotodor-envoy_balanceador-image:p3 -f ./P3-flotodor-envoy/DockerfileEnvoy_balanceador .
      ;;
    all)
      build_image apache
      build_image nginx
...
# Se espera que el archivo de configuración esté en la ruta ./P3-flotodor-haproxy/config_balanceador/
...
function set_haproxy_strategy() {
  ...
  case "$strategy" in
    lc)
      cp ./P3-flotodor-haproxy/config_balanceador/haproxy_lc.cfg ./P3-flotodor-haproxy/config_balanceador/haproxy.cfg
      echo -e "${blueColour}[i] Estrategia de balanceo: menor número de conexiones${endColour}"
      ;;
    rb|"")
      cp ./P3-flotodor-haproxy/config_balanceador/haproxy_rb.cfg ./P3-flotodor-haproxy/config_balanceador/haproxy.cfg
...
# Se copia el archivo de configuración correspondiente a la estrategia seleccionada
# y se muestra un mensaje indicando la estrategia utilizada
# Se espera que el archivo de configuración esté en la ruta ./P3-flotodor-nginx/config_balanceador/
...
function set_nginx_strategy() {
  ...
  case "$strategy" in
    pd)
      cp ./P3-flotodor-nginx/config_balanceador/nginx_pd.conf ./P3-flotodor-nginx/config_balanceador/nginx.conf
      echo -e "${blueColour}[i] Estrategia de balanceo: ponderación con pesos${endColour}"
      ;;
    rb|"")
      cp ./P3-flotodor-nginx/config_balanceador/nginx_rb.conf ./P3-flotodor-nginx/config_balanceador/nginx.conf
      echo -e "${blueColour}[i] Estrategia de balanceo: round-robin (por defecto)${endColour}"
      ;;
    *)
