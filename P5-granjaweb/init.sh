...
function build_image(){
  case $1 in
    ...
    envoy_balanceador)
      ...
      ;;
    ab)
      docker rmi flotodor-ab-image:p5 -f
      docker build --no-cache -t flotodor-ab-image:p5 -f ./P5-ab/DockerfileAB .
      ;;
    all)
      build_image apache
      build_image nginx
      ...
      ;;
    *)
      echo -e "${redColour}[!] Selecciona: apache, nginx,nginx_balanceador, haproxy_balanceador, traefik_balanceador, envoy_balanceador, ab o all${endColour}"
      ;;
  esac
}
...
function stop_conflicting_balanceador() {
 local selected=$1
  # Si no es un balanceador conocido, no toques nada
  [[ ! "$selected" =~ ^(nginx|haproxy|traefik|envoy)$ ]] && return
